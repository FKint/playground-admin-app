...
<script>
    $(function () {
        ...
        const table = table_element.DataTable({
            ...
            dom: 'Blfrtip',
            serverSide: false,
            ajax: '{!! route('getListParticipants', ['list_id'=>$list->id]) !!}',
            buttons: [
                {
                    extend: 'pdfHtml5',
                    exportOptions: {
        ...
        table_element.on('click', '.btn-remove-child-family-list', function () {
            ...
            $.post('{{ route('removeListParticipant', ['list_id' => $list->id]) }}',
        ...
        const engine = new Bloodhound({
            remote: {
                url: '{!! route('getListChildFamilySuggestions', ['list_id' => $list->id]) !!}?q=%QUERY%',
                wildcard: '%QUERY%',
                cache: false
            },
